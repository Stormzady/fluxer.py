...
This is a list of Frequently Asked Questions regarding using ``fluxer.py`` and its extension modules. Feel free to suggest a
...
Example code can be found in the `examples folder <https://github.com/Rapptz/fluxer.py/tree/master/examples>`_
in the repository.
...
`Check the background_task.py example. <https://github.com/Rapptz/fluxer.py/blob/master/examples/background_task.py>`_
...
2. Did you invite your bot with the correct permissions? Bots need to be invited with the ``applications.commands``
   ...
   ``https://fluxer.app/oauth2/authorize?client_id=<client id>&scope=applications.commands+bot``.
